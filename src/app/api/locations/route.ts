...
export async function GET() {
  ...
  const findLocationEquipments = dataLocation.map(equipment => {
    const equipmentsLocations = equipment.positions[equipment.positions.length -1]

    const name = dataEquipment.filter(equipmentData => equipmentData.id === equipment.equipmentId)
