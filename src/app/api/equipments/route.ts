...
export async function GET() {
  ...
  const getEquipments = data.filter((equipment) => equipment.id)
    ...
    const mergedEquipments = getEquipments.map(equipment => {
      ...
      const model = dataModels.find(model => model.id === equipment.equipmentModelId)
      //Buscar status atual do equipamento
      const modelState = activeStates.find(modelState => modelState.equipmentId === equipment.id)
      ...
      const labelState = statesTypes.find(label => label.id === (lastEquipmentId?.equipmentStateId ?? ''));
      return {
        ...equipment,
